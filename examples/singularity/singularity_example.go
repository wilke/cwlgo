...
func main() {
	...
	message := "Hello from Singularity container!"
	if len(os.Args) > 1 {
		message = os.Args[1]
	}
	...
	// Parse the CWL file
	cwlFile := filepath.Join(dir, "singularity_example.cwl")
	...
	// Make sure Singularity is enabled
	executor.SingularityEnabled = true

	fmt.Println("Executing tool in Singularity container...")
